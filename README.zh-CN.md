...
从 crates.io 安装已经发布的 `meco-core 0.5.0`：

```sh
cargo install meco-core --version 0.5.0 --locked
```
...
UTN #57 路径使用 `zvvnmod-utn57 0.3.2` 中 reviewed 的 ZVVNMOD → positioned written unit mapping，以及固定版本的纯 Rust 归一化库 `mongol-norm 0.2.1`。两者都已编译进 `meco`，不需要额外安装任何东西。
...
从 [v0.5.0 Release](https://github.com/Satsrag/meco-rust/releases/tag/v0.5.0) 下载。
...
| 浏览器/WebAssembly | `meco-wasm-web-0.5.0.tgz` |
| Node.js/WebAssembly | `meco-wasm-nodejs-0.5.0.tgz` |
