...
const work = mkdtempSync(join(tmpdir(), 'meco-skill-test-'));
...
const manifest = JSON.parse(readFileSync(join(skill, 'assets/meco/manifest.json'), 'utf8'));
const draft = 'ᠤᠯᠤᠰ ᠤᠨ\nᠰᠠᠶᠢᠨ 2026';
...
try {
  for (const [name, expected] of Object.entries(manifest.files_sha256)) {
    const bytes = readFileSync(join(skill, 'assets/meco', name));
    assert.equal(createHash('sha256').update(bytes).digest('hex'), expected, name);
  }
  for (const dependency of manifest.dependencies) {
    for (const name of dependency.license_files) {
      assert.ok(readFileSync(join(skill, 'assets/meco/licenses', name)).length, name);
    }
  }
  assert.equal(run(['--version']).stdout, `meco ${manifest.meco_version} (bundled WebAssembly)\n`);
  ...
  for (const target of ['utn57', 'menk_shape', 'menk_letter', 'delehi', 'z52', 'zvvnmod', 'utn57_shape']) {
    ...
    assert.equal(report.meco_version, manifest.meco_version);
